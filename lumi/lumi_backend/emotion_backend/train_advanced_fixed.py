"""
Fixed train_advanced.py with ExpW support
Key fixes:
1. Fixed CrossEntropyLoss typo
2. Added F import
3. Auto-detects ExpW vs folder format
"""
...
# Import your modules
from src.dataset import FERImageFolder, ExpWDataset, get_transforms, CLASS_NAMES
...
class AdvancedTrainer:
    ...
    def setup_data(self):
        """Setup datasets - Auto-detect ExpW vs folder format"""
        print("\n📦 Loading datasets...")
        ...
        if os.path.exists(os.path.join(self.args.train_dir, 'label.lst')):
            print("📦 Detected ExpW dataset format")
            self.train_dataset = ExpWDataset(
                self.args.train_dir,
                transform=train_transform
            )
            self.val_dataset = ExpWDataset(
                self.args.val_dir,
                transform=val_transform
            )
        else:
            print("📦 Using standard folder format")
    ...
    def setup_training(self):
        ...
        if self.args.loss == 'focal':
            ...
            if self.class_weights is not None:
                self.criterion = nn.CrossEntropyLoss(weight=self.class_weights)  # ✅ FIXED
                print("✅ Using Weighted CrossEntropy")
            else:
                self.criterion = nn.CrossEntropyLoss()  # ✅ FIXED
